...
interface FilterControlsProps {
    filters: FilterState;
    onPriceOptionChange: (optionId: string, checked: boolean) => void;
    onSliderChange: (value: [number, number]) => void;
    onFlavourChange: (id: string, checked: boolean) => void;
    onOccasionChange: (id: string, checked: boolean) => void;
    onProductTypeChange: (id: string, checked: boolean) => void;
    onWeightChange: (id: string, checked: boolean) => void;
    isMobile?: boolean;
}
            ...
            <div className="space-y-2 pl-0">
              {priceOptions.map((option) => (
                 <CheckboxItem
                    ...
                    onCheckedChange={(checked) => onPriceOptionChange(option.id, !!checked)}
                  />
              ))}
            </div>
            <FilterSection title="Flavours & Fillings" icon={<Image src="/icons/flavours_icon.png" alt="Flavours" width={18} height={18} onDragStart={(e) => e.preventDefault()}/>}>
              {flavourOptions.map((option) => (
                <CheckboxItem
                  ...
                  onCheckedChange={(checked) => onFlavourChange(option.id, !!checked)}
                />
              ))}
            </FilterSection>
            <FilterSection title="Best for Occasion" icon={<Image src="/icons/occasion_icon.png" alt="Occasion" width={18} height={18} onDragStart={(e) => e.preventDefault()}/>}>
              {occasionOptions.map((option) => (
                <CheckboxItem
                  ...
                  onCheckedChange={(checked) => onOccasionChange(option.id, !!checked)}
                />
              ))}
            </FilterSection>
            <FilterSection title="Product Type" icon={<Image src="/icons/product_type_icon.png" alt="Product Type" width={16} height={16} onDragStart={(e) => e.preventDefault()}/>}>
              {productTypeOptions.map((option) => (
                <CheckboxItem
                  ...
                  onCheckedChange={(checked) => onProductTypeChange(option.id, !!checked)}
                />
              ))}
            </FilterSection>
            <FilterSection title="Weight" icon={<Image src="/icons/weight_icon.png" alt="Weight" width={14} height={14} onDragStart={(e) => e.preventDefault()}/>}>
              {weightOptions.map((option) => (
                <CheckboxItem
                  ...
                  onCheckedChange={(checked) => onWeightChange(option.id, !!checked)}
                />
              ))}
            </FilterSection>
          </div>
