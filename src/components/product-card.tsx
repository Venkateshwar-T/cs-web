...
export function ProductCard({
  ...
  return (
    <div
      ...
      className="relative w-full h-full bg-white/90 rounded-2xl overflow-hidden cursor-pointer group border border-white/20 hover:border-custom-gold/50 transition-colors duration-300 flex flex-col shadow-lg"
    >
      {/* Image Section */}
      ...
      {/* Details Section */}
      <div className="flex flex-col p-3 md:p-4 text-black flex-grow">
        ...
        <div className="flex justify-between items-center mt-2">
          {product.discountedPrice !== undefined && (
            <div className="flex flex-col">
              ...
              <p className="font-bold text-sm md:text-lg text-custom-purple-dark -mt-1">₹{product.discountedPrice.toFixed(2)}</p>
            </div>
          )}
          ...
          <div className="flex-shrink-0">
            {quantity === 0 ? (
              ...
              <div className="flex items-center justify-center h-8 md:h-9 w-24 rounded-full bg-custom-purple-dark text-white overflow-hidden">
                <Button variant="ghost" size="icon" onClick={handleDecrement} className="h-full rounded-none flex-1 hover:bg-black/20 text-white hover:text-white"><Minus className="h-4 w-4" /></Button>
                <span className="font-bold text-sm flex-1 text-center">{quantity}</span>
                <Button variant="ghost" size="icon" onClick={handleIncrement} className="h-full rounded-none flex-1 hover:bg-black/20 text-white hover:text-white"><Plus className="h-4 w-4" /></Button>
              </div>
            )}
          </div>
        </div>
